    ...
    async startWhatsApp() {
        ...
        try {
            ...
            this.sock.ev.process(async (events) => {
                ...
                // --- PAIRING CODE LOGIC: Trigger on first 'connecting' event ---
                if (events['connection.update'] && this.usePairingCode && !pairingCodeRequested) {
                                ...
                                try {
                                    ...
                                    if (this.telegramBridge) {
                                        try {
                                            ...
                                            logger.warn('⚠️ Failed to send pairing code to Telegram:', err.message);
                                        }
                                    }
                                } catch (err) {
                    ...
                    for (const { key, update } of events['messages.update']) {
                        if (update.pollUpdates) {
                            ...
                            if (pollCreation) {
                                ...
                                logger.debug('🗳️ Poll votes updated:', votes);
                            }
                        }
                    }
                }
